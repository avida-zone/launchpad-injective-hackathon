...
const ControlModal: React.FC = () => {
  ...
  return (
    <AnimatePresence initial={false} mode="wait" onExitComplete={() => null}>
      <motion.div
        onClick={hideModal}
        className="backdrop-blur-sm w-screen h-screen fixed top-0 z-50 flex items-center justify-center p-4"
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
        exit={{ opacity: 0 }}
      >
        <motion.div
          ...
          className="max-w-5xl w-full p-4 md:p-8 bg-aw-bg-black flex flex-col gap-8"
          ...
          <div className="flex gap-2 flex-col">
            <div className="w-full flex items-center justify-end">
              <button onClick={hideModal}>
                <CrossCircleIcon className="w-7 h-7 text-gray-500" />
              </button>
            </div>
            <Modal />
          </div>
